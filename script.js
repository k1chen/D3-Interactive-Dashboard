d3.csv("filtered_data_with_rowName_and_colName.csv").then(function (data) {
  const defaultYear = data[0].Year.toString();
  populateYearDropdown(data);
  updateBarChart(data, defaultYear);
  updatePieChart(data, defaultYear);
...
function updateBarChart(data, selectedYear, filteredData = null) {
  ...
  dataToUse.forEach((d) => {
    let bipocCategory = bipocCategories.includes(d.BIPOC)
      ? d.BIPOC
      : "All White"; 
  ...
  const stack = d3
    ...
    .value((d, key) => {
      let matchingElement = d.find((item) => item.key.includes(key));
      return matchingElement ? matchingElement.value : 0;
    });
